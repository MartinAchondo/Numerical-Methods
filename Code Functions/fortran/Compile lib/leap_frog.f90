...
subroutine leap_frog(u_out,dt,dx,gamma,Nt,Nx,f,u_0) bind(C, name="leap_frog")
    ...
    real(c_double) :: lambda,t,x,v0(Nx)
    ...
    lambda = (gamma*dt/dx)**2


    do i=2,Nx-1
        x = (i-1)*dx
        u_old(i) = u_old2(i) + dt*v0(i) +(lambda/2.0d0)*(u_old2(i+1) - 2.0d0*u_old2(i) + u_old2(i-1) + (dt**2)*func(t,x,f,gamma))
        u_out(2,i) = u_old(i)
    end do
    ...
    do n_t=3,Nt
        ...
        do j=2,Nx-1
            x = (j-1)*dx
            u(j) = 2.0d0*u_old(j) - u_old2(j) + lambda*(u_old(j+1)-2.0d0*u_old(j)+u_old(j-1)) + (dt**2)*func(t,x,f,gamma)
            u_out(n_t,j) = u(j)
