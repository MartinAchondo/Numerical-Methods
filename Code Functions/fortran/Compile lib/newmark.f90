...
subroutine newmark(u_out,dt,dx,gamma,Nt,Nx,f,beta,theta,u_0) bind(C, name="newmark")
    ...
    do n_t=2,Nt
        ...
        do j=2,Nx-1
            ...
            w(j) = (1.0d0/dx**2)*(u_old(j+1)-2.0d0*u_old(j)+u_old(j-1))

            v(j) = v_old(j)+dt*( (1-theta)*(gamma**2*w_old(j)+func(t-dt,x,f,gamma))+theta*(gamma**2*w(j)+func(t,x,f,gamma)))

            b(j-1) = u_old(j) + dt*v(j) + dt**2*beta*func(t+dt,x,f,gamma) + (0.5d0-beta)*dt**2*(gamma**2*w(j) + func(t,x,f,gamma) )
